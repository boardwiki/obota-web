<script setup lang="ts">
    ...
    const filter_footPlant = ref<any>(null);
    const filter_flip = ref<any>('');
    const filter_body = ref<string[]>([]);
    const filter_board = ref<string[]>([]);
    ...
    onMounted(async()=>{
        ...
        filter_difficulty.value = stoFoot.filter.difficulty ? stoFoot.filter.difficulty : null;
        filter_footPlant.value = stoFoot.filter.footPlant ? stoFoot.filter.footPlant : null;
        filter_flip.value = stoFoot.filter.flip ? stoFoot.filter.flip : null;
        filter_keyword.value = stoFoot.filter.keyword? stoFoot.filter.keyword : null;
    })

    const reset = () => {
        filter_difficulty.value = null;
        filter_footPlant.value = null;
        filter_flip.value = null;
    ...
    const set = async() => {
        stoFoot.filter.difficulty = filter_difficulty.value ? filter_difficulty.value : null;
        stoFoot.filter.footPlant  = filter_footPlant.value !== -1 ? filter_footPlant.value : null;
        stoFoot.filter.flip       = filter_flip.value;  
        stoFoot.filter.keyword    = filter_keyword.value;
        /*
        stoFoot.filter.bodyDegree = filter_body.value;
        stoFoot.filter.boardDegree = filter_board.value;
        */
    ...
    <div class="skill_search">
        <div class="btn_wrap">
            <button type="button" class="fliter_title_btn" @click="show_title=true">
                ...
                <span aria-hidden="true">{{stoFoot.filter.keyword}}</span>
