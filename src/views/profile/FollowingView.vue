<script setup lang="ts">
    ...
    function toggleFollow(id:number,amIFollowing:boolean){
        const userId = following.value[id].id;
        following.value[id].amIFollowing = !amIFollowing
        if(amIFollowing){
    ...
    const find = async() =>{
        await stoProfile.getFollowing(Number(props.id),user_search.value);
    ...
    onMounted(async()=>{
        if(props.id){
            ...
            await stoProfile.getFollowing(id,user_search.value);
...
<template>
    <main class="follow_view">
        ...
        <div class="follow_user_list">
            ...
            <ul>
                <li v-for="(user,id) in following" :key="id">
            ...
            <Empty v-if="!stoProfile.following" msg="유저가 없습니다."></Empty>
        </div>
